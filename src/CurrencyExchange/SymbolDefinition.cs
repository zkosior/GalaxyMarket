namespace GalaxyMarket.CurrencyExchange
{
	...
	public class SymbolDefinition
	{
		...
		public string this[string key] => this.definitions.ContainsKey(key)
			...
			: throw new ArgumentException($"Intergalactic unit not registered: {key}");

		public void AddDefinition(string unit, string definition)
		{
			if (!this.whitelistedNumerals.Contains(definition))
			{
				throw new ArgumentException($"Not supported Roman Numeral: {definition}");
			}

			if (this.definitions.ContainsKey(unit))
			{
				throw new ArgumentException($"Intergalactic unit already registered: {unit}");
			}

			if (this.definitions.ContainsValue(definition))
			{
				throw new ArgumentException($"Roman numeral already registered: {definition}");
			}
